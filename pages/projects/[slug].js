...
const ProjectSection = ({ section }) => {
  return (
    <>
      ...
      <div className="content-container mt-10 pb-10 grid md:grid-cols-2 gap-10 xl:gap-16">
        <div className="flex flex-col gap-10 xl:gap-16">
          {section.images.map(
            (image, index) =>
              index % 2 == 0 && <img key={index} src={image} className="h-auto max-w-full rounded-4xl" />
          )}
        </div>
        <div className="flex flex-col gap-10 xl:gap-16">
          {section.images.map(
